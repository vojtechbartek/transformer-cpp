...
int main() {
  ...
  std::vector<std::vector<std::vector<float>>> output =
      forward_layer.forward(input);

  // check dimensions of the output
  if (output.size() != batch_size || output[0].size() != seq_len ||
      output[0][0].size() != output_dim) {
    std::cerr << "Output dimensions are not correct" << std::endl;
    std::cerr << "Expected: " << batch_size << "x" << seq_len << "x"
              << output_dim << std::endl;
    std::cerr << "Got: " << output.size() << "x" << output[0].size() << "x"
  ...
  */
  ...
  std::vector<std::vector<std::vector<float>>> grad_output(
      batch_size, std::vector<std::vector<float>>(
                      seq_len, std::vector<float>(output_dim, 0.5f)));

  std::vector<std::vector<std::vector<float>>> grad_input =
      forward_layer.backward(input, grad_output);

  // check dimensions of the grad_input
  if (grad_input.size() != batch_size || grad_input[0].size() != seq_len ||
      grad_input[0][0].size() != input_dim) {
    std::cerr << "Grad input dimensions are not correct" << std::endl;
    std::cerr << "Expected: " << batch_size << "x" << seq_len << "x"
              << input_dim << std::endl;
    std::cerr << "Got: " << grad_input.size() << "x" << grad_input[0].size()
              << "x" << grad_input[0][0].size() << std::endl;
    return 1;
  }

  // check weight gradients dimensions
  std::vector<std::vector<std::vector<float>>> grad_weights =
      forward_layer.get_grad_weights();
  ...
  if (grad_W1.size() != input_dim || grad_W1[0].size() != hidden_dim) {
    ...
    std::cerr << "Got: " << grad_W1.size() << "x" << grad_W1[0].size()
              << std::endl;
    return 1;
  }
  ...
  if (grad_W2.size() != hidden_dim || grad_W2[0].size() != output_dim) {
    ...
    std::cerr << "Got: " << grad_W2.size() << "x" << grad_W2[0].size()
              << std::endl;
    return 1;
  }
