...
bool checkPositionalEncodingProperties(
    std::vector<std::vector<float>> &positionalEncoding, int length,
    int embeddingDim) {
...
bool checkPositionalEncodingValue(
    std::vector<std::vector<float>> &positionalEncoding, int length,
    int embeddingDim) {
  ...
  if (positionalEncoding[0][0] != 0.0) {
    std::cerr << "Error: positionalEncoding[0][0] = "
              << positionalEncoding[0][0] << " != 0.0" << std::endl;
    return false;
  }
  ...
  if (positionalEncoding[0][1] != 1.0) {
    std::cerr << "Error: positionalEncoding[0][1] = "
              << positionalEncoding[0][1] << " != 1.0" << std::endl;
    return false;
  }
  ...
  // 2nd column
  // sin (2 / 10000^((2*2)/512))
  if (!floatAlmostEqual(positionalEncoding[2][2],
                        std::sin(2 / std::pow(10000, 4.0 / 512)))) {
    std::cerr << "Error: positionalEncoding[2][2] = "
              << positionalEncoding[2][2] << " != sin(2 / 10000^((2*2)/"
              << embeddingDim << "))" << " = "
              << std::sin(2 / std::pow(10000, 4.0 / 512)) << std::endl;
    return false;
  }
...
int main() {
  ...
  std::vector<std::vector<float>> positionalEncoding =
      PositionalEncoding::generate(length, embedding_dim);
  if (checkPositionalEncodingProperties(positionalEncoding, length,
                                        embedding_dim)) {
