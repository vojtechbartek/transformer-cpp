...
int main() {
  ...
  std::vector<std::vector<int>> matrix1 = {
      {1, 2, 3}, {4, -2, 6}, {7, 0, 9}, {1, 2, 1}};

  std::vector<std::vector<int>> matrix2 = {
      {1, 0, 2, 1}, {1, 2, 3, 2}, {0, 1, 3, -2}};

  std::vector<std::vector<int>> expected_result = {
      {3, 7, 17, -1}, {2, 2, 20, -12}, {7, 9, 41, -11}, {3, 5, 11, 3}};
  // test matrix multiplication
  std::vector<std::vector<int>> result =
      MatrixUtils::matrixMultiplication(matrix1, matrix2);
  bool same = true;
  ...
  std::vector<std::vector<float>> matrix3 = {
      {1.2f, 2.0f}, {4.1f, -2.2f}, {7.9f, 0.0f}};

  std::vector<std::vector<float>> expected_transposed = {{1.2f, 4.1f, 7.9f},
                                                         {2.0f, -2.2f, 0.0f}};

  std::vector<std::vector<float>> transposed =
      MatrixUtils::matrixTranspose(matrix3);
  ...
  std::vector<std::vector<float>> result_softmax =
      MatrixUtils::rowSoftmax(matrix4);
  bool softmaxed = true;
  for (int i = 0; i < result_softmax.size(); i++) {
    ...
    for (int j = 0; j < result_softmax[i].size(); j++) {
      ...
      if (result_softmax[i][j] < 0.0f || result_softmax[i][j] > 1.0f) {
        softmaxed = false;
        std::cout
            << "[-] Matrix softmax test failed, value out of range [0,1] : "
            << result_softmax[i][j] << std::endl;
        break;
      }
    ...
    if (std::abs(sum - 1.0f) > EPS) {
      softmaxed = false;
      std::cout << "[-] Matrix softmax test failed, sum of row is not 1 : "
                << sum << std::endl;
      break;
    }
  }
  ...
  std::vector<std::vector<float>> matrix5 = {
      {1.0f, 2.1f, 3}, {4, -2, 6}, {7, 0, 9}, {1, 2, 10000}};

  std::vector<std::vector<float>> matrix6 = {
      {1, 0.0f, 2}, {1, 2.0f, 3}, {0.1f, 1, 3.0f}, {1, 2, 1}};

  std::vector<std::vector<float>> expected_result_addition = {
      {2.0f, 2.1f, 5}, {5, 0, 9}, {7.1f, 1, 12}, {2, 4, 10001}};

  std::vector<std::vector<float>> result_addition =
      MatrixUtils::matrixAddition(matrix5, matrix6);
  ...
  std::vector<std::vector<std::vector<float>>> tensor = {
      {{1.0f, 2.1f}, {4, -2}, {7, 0}}, {{1, 2}, {4, -2}, {3, 0}}};

  std::vector<std::vector<float>> expected_result_mean = {
      {1.0f, 2.05f}, {4, -2}, {5, 0}};

  std::vector<std::vector<float>> result_mean =
      MatrixUtils::batchMatrixMean(tensor);
  bool same_mean = true;
  ...
  for (int i = 0; i < result_mean_vector.size(); i++) {
    if (std::abs(result_mean_vector[i] - expected_result_mean_vector[i]) >
        EPS) {
  ...
  std::vector<std::vector<float>> grad_output = {{1.0f, 2.0f}, {3.0f, 4.0f}};
  std::vector<std::vector<float>> softmax_output = {{0.3543f, 0.6457f},
                                                    {0.5498f, 0.4502f}};

  // Expected matrix
  std::vector<std::vector<float>> expected_softmax_der_output = {
      {-0.2288f, 0.2288f}, {-0.2475f, 0.2475f}};
  std::vector<std::vector<float>> result_softmax_der =
      MatrixUtils::rowSoftmaxDerivative(grad_output, softmax_output);
  bool same_softmax_der = true;
  // Check dimensions
  if (result_softmax_der.size() != expected_softmax_der_output.size() ||
      result_softmax_der[0].size() != expected_softmax_der_output[0].size()) {
    std::cout << "Softmax derivative test dimensions do not match" << std::endl;
    std::cout << "Expected : " << expected_softmax_der_output.size() << "x"
              << expected_softmax_der_output[0].size()
              << " got : " << result_softmax_der.size() << "x"
              << result_softmax_der[0].size() << std::endl;

    same_softmax_der = false;
  }
  for (int i = 0; i < result_softmax_der.size(); i++) {
    for (int j = 0; j < result_softmax_der[i].size(); j++) {
      if (std::abs(result_softmax_der[i][j] -
                   expected_softmax_der_output[i][j]) > EPS) {
        same_softmax_der = false;
        std::cout << "at position (" << i << ", " << j
                  << ") expected : " << expected_softmax_der_output[i][j]
                  << " got : " << result_softmax_der[i][j] << std::endl;
        break;
      }
    }
  }
