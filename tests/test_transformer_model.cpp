...
int main() {
  ...
  std::vector<std::vector<std::vector<float>>> input_tensor(
      batch_size, std::vector<std::vector<float>>(
                      seq_len, std::vector<float>(embedding_dim, 1.0)));

  // Test forward pass
  auto output = model.forward(input_tensor);
  std::cout << "Output shape: " << output.size() << " x " << output[0].size()
            << " x " << output[0][0].size() << std::endl;
  ...
  for (int b = 0; b < batch_size; b++) {
    for (int s = 0; s < seq_len; s++) {
      ...
      if (std::abs(sum - 1) > 1e-4) {
        ...
        std::cout << "Got sum: " << sum << " at position " << b << " x " << s
                  << std::endl;
        test_passed = false;
        break;
      }
  ...
  if (output.size() != batch_size || output[0].size() != seq_len ||
      output[0][0].size() != vocab_size) {
    std::cout << "[-] Forward shape test failed" << std::endl;
    std::cout << "Expected shape: " << batch_size << " x " << seq_len << " x "
              << vocab_size << std::endl;
    std::cout << "Got shape: " << output.size() << " x " << output[0].size()
              << " x " << output[0][0].size() << std::endl;
  } else {
  ...
  std::vector<std::vector<std::vector<float>>> grad_output_tensor(
      batch_size, std::vector<std::vector<float>>(
                      seq_len, std::vector<float>(vocab_size, 0.6)));
  ...
  std::cout << "Grad output shape: " << grad_output.size() << " x "
            << grad_output[0].size() << " x " << grad_output[0][0].size()
            << std::endl;
  if (grad_output.size() != batch_size || grad_output[0].size() != seq_len ||
      grad_output[0][0].size() != embedding_dim) {
    std::cout << "[-] Backward test failed" << std::endl;
    std::cout << "Expected shape: " << batch_size << " x " << seq_len << " x "
              << vocab_size << std::endl;
    std::cout << "Got shape: " << grad_output.size() << " x "
              << grad_output[0].size() << " x " << grad_output[0][0].size()
              << std::endl;
  } else {
