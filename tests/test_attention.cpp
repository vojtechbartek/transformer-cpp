...
int main() {
  ...
  std::vector<std::vector<std::vector<float>>> input(
      ...
      std::vector<std::vector<float>>(
          seq_len, std::vector<float>(embed_dim, 1.0f))); // Dummy input
  std::vector<std::vector<std::vector<float>>> mask(
      batch_size,
      std::vector<std::vector<float>>(
          seq_len, std::vector<float>(embed_dim, 1.0f))); // Dummy mask

  SelfAttention selfAttention(embed_dim, head_size);
  std::vector<std::vector<std::vector<float>>> output =
      selfAttention.forward(input, mask);

  if (output.size() != batch_size || output[0].size() != seq_len ||
      output[0][0].size() != head_size) {
    std::cerr << "[-] Self-attention Forward shape test failed" << std::endl;
    std::cerr << "Expected: (" << batch_size << ", " << seq_len << ", "
              << head_size << ")" << std::endl;
    std::cerr << "Got: (" << output.size() << ", " << output[0].size() << ", "
  ...
  std::vector<std::vector<std::vector<float>>> softmax_output =
      selfAttention.getSoftmaxOutput();
  ...
  std::vector<std::vector<std::vector<float>>> grad_output(
      batch_size,
      std::vector<std::vector<float>>(
          seq_len, std::vector<float>(head_size, 1.0f))); // Dummy grad_output

  std::vector<std::vector<std::vector<float>>> grad_input =
      selfAttention.backward(input, grad_output);
  if (grad_input.size() != batch_size || grad_input[0].size() != seq_len ||
      grad_input[0][0].size() != embed_dim) {
    std::cerr << "[-] Self-attention Backward shape test failed" << std::endl;
    std::cerr << "Expected: (" << batch_size << ", " << seq_len << ", "
              << embed_dim << ")" << std::endl;
    std::cerr << "Got: (" << grad_input.size() << ", " << grad_input[0].size()
              << ", " << grad_input[0][0].size() << ")" << std::endl;
  } else {
