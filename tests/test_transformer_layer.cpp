...
int main() {
  ...
  TransformerLayer transformer_layer(seq_len, embed_dim, head_size,
                                     ff_hidden_dim, output_dim);
  ...
  std::vector<std::vector<std::vector<float>>> output =
      transformer_layer.forward(input);
  ...
  if (output.size() == batch_size && output[0].size() == seq_len &&
    ...
    std::cout << "Expected: " << batch_size << " x " << seq_len << " x "
              << output_dim << std::endl;
    std::cout << "Got: " << output.size() << " x " << output[0].size() << " x "
  ...
  std::vector<std::vector<std::vector<float>>> grad_input =
      transformer_layer.backward(grad_output);
  ...
  if (grad_input.size() == batch_size && grad_input[0].size() == seq_len &&
    ...
    std::cout << "[+] Transformer layer grad_input shape test passed"
              << std::endl;
  } else {
    std::cout << "[-] Transformer layer grad_input shape test failed"
              << std::endl;
    std::cout << "Expected: " << batch_size << " x " << seq_len << " x "
              << embed_dim << std::endl;
    std::cout << "Got: " << grad_input.size() << " x " << grad_input[0].size()
