    ...
    for (size_t j = 0; j < a[i].size(); j++) {
      for (size_t k = 0; k < a[i][j].size(); k++) {
        if (std::abs(a[i][j][k] - b[i][j][k]) > EPSILON) {
          std::cout << "Different values: a[" << i << "][" << j << "][" << k
                    << "] = " << a[i][j][k] << " but b[" << i << "][" << j
                    << "][" << k << "] = " << b[i][j][k] << std::endl;
          return false;
        }
  ...
  for (size_t i = 0; i < a.size(); i++) {
    for (size_t j = 0; j < a[i].size(); j++) {
      if (std::abs(a[i][j] - b[i][j]) > EPSILON) {
        std::cout << "Different values: a[" << i << "][" << j
                  << "] = " << a[i][j] << " but b[" << i << "][" << j
                  << "] = " << b[i][j] << std::endl;
        return false;
      }
...
std::vector<std::vector<std::vector<float>>> create_random_matrix(int b, int m,
                                                                  int k) {
  std::random_device rd;
  std::default_random_engine generator(rd());
  std::uniform_real_distribution<float> distribution(-1.0, 1.0);
  ...
  std::vector<std::vector<std::vector<float>>> matrix(
      b, std::vector<std::vector<float>>(m, std::vector<float>(k)));
  for (int i = 0; i < b; i++) {
    for (int j = 0; j < m; j++) {
      for (int l = 0; l < k; l++) {
...
bool random_matrix_multiplication3D_test(int b, int m, int k, int p) {
  std::vector<std::vector<std::vector<float>>> A =
      create_random_matrix(b, m, k);
  std::vector<std::vector<std::vector<float>>> B =
      create_random_matrix(b, k, p);

  std::vector<std::vector<std::vector<float>>> C_cuda =
      CudaHelpers::batchMatrixMultiplication(A, B);
  std::vector<std::vector<std::vector<float>>> C_cpu =
      MatrixUtils::batchMatrixMultiplication(A, B);
...
bool random_matrix_multiplication2D_test(int m, int k, int p) {
  ...
  std::vector<std::vector<float>> C_cuda =
      CudaHelpers::matrixMultiplication(A, B);
  std::vector<std::vector<float>> C_cpu =
      MatrixUtils::matrixMultiplication(A, B);
...
bool random_matrix_addition3D_test(int b, int m, int n) {
  std::vector<std::vector<std::vector<float>>> A =
      create_random_matrix(b, m, n);
  std::vector<std::vector<std::vector<float>>> B =
      create_random_matrix(b, m, n);

  std::vector<std::vector<std::vector<float>>> C_cuda =
      CudaHelpers::matrixAddition(A, B);
  std::vector<std::vector<std::vector<float>>> C_cpu =
      MatrixUtils::matrixAddition(A, B);
...
bool random_batch_row_softmax_test(int b, int m, int n) {
  std::vector<std::vector<std::vector<float>>> A =
      create_random_matrix(b, m, n);
  std::vector<std::vector<std::vector<float>>> C_cuda =
      CudaHelpers::rowSoftmax(A);
  std::vector<std::vector<std::vector<float>>> C_cpu =
      MatrixUtils::rowSoftmax(A);
...
bool random_row_softmax_derivative_test(int m, int n) {
  ...
  std::vector<std::vector<float>> softmax_output =
      MatrixUtils::rowSoftmax(softmax_input);
  std::vector<std::vector<float>> grad_output = create_random_matrix(m, n);

  std::vector<std::vector<float>> C_cuda =
      CudaHelpers::rowSoftmaxDerivative(softmax_output, grad_output);
  std::vector<std::vector<float>> C_cpu =
      MatrixUtils::rowSoftmaxDerivative(softmax_output, grad_output);

  return all_close(C_cuda, C_cpu);
}

bool random_batch_row_softmax_derivative_test(int b, int m, int n) {
  std::vector<std::vector<std::vector<float>>> softmax_input =
      create_random_matrix(b, m, n);
...
bool random_batch_matrix_transpose_test(int b, int m, int n) {
  std::vector<std::vector<std::vector<float>>> A =
      create_random_matrix(b, m, n);
  std::vector<std::vector<std::vector<float>>> C_cuda =
      CudaHelpers::batchMatrixTranspose(A);
  std::vector<std::vector<std::vector<float>>> C_cpu =
      MatrixUtils::batchMatrixTranspose(A);
...
int main() {
  ...
  for (int i = 0; i < num_tests; i++) {
    ...
    if (!random_row_softmax_derivative_test(m, k)) {
      std::cout << "[-] Row-wise Softmax Derivative 2D Test failed"
                << std::endl;
      return 1;
    }
  }
  ...
  for (int i = 0; i < num_tests; i++) {
    ...
    if (!random_batch_row_softmax_derivative_test(b, m, k)) {
      std::cout << "[-] Row-wise Softmax Derivative 3D Test failed"
                << std::endl;
      return 1;
    }
  }
