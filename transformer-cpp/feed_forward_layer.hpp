...
private:
	...
	std::vector<std::vector<std::vector<float>>> hidden_relu; // output of ReLU, used in backward
	
	std::vector<std::vector<float>> W1, W2; // weights, not batched
	std::vector<float> b1, b2; // biases, not batched
	
	std::vector<std::vector<float>> grad_W1, grad_W2; // gradients of weights, not batched
