...
int main() {
    ...
    for (int epoch = 0; epoch < num_epochs; ++epoch) {
        ...
        // At each step in the epoch, we will load batch_size * seq_len samples 
        // from the input data
        int step = 0;
        while (step < (input_data.size() - batch_size * seq_len)) {
            ...
            std::vector<std::vector<std::vector<float>>> 
                grad_output(output_logits.size(), 
                    std::vector<std::vector<float>>(output_logits[0].size(),
            ...
            for (int i = 0; i < batch_size; i++) {
                ...
                for (int j = 0; j < output_logits[0].size(); j++) {
                    grad_output[i][j][target_data_batched[i][j]] = -1.0 / output_logits[i][j][target_data_batched[i][j]];
                }
            }
