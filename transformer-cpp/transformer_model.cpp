...
Transformer::Transformer(const YAML::Node& config) {
    ...
    std::vector<int> ff_hidden_dims = config["ff_hidden_dims"].as<std::vector<int>>();
    std::vector<int> head_sizes = config["head_sizes"].as<std::vector<int>>();
    std::vector<int> layer_output_dims = config["layer_output_dims"].as<std::vector<int>>();
    
    // Initialize the transformer layers
    // First layer takes in the input embeddings and the rest take in the output of the previous layer
    // First layer also takes in the positional encodings
    
    layers.push_back(TransformerLayer(seq_len, embedding_dim, head_sizes[0], ff_hidden_dims[0], layer_output_dims[0]));

    // Middle layers until the second to last layer
    for (int i = 1; i < num_layers - 1; i++) {
        layers.push_back(TransformerLayer(seq_len, layer_output_dims[i-1], head_sizes[i], ff_hidden_dims[i], layer_output_dims[i]));
    }

    // Last layer
    // The last layer outputs vector of size vocab_size
    // This is then passed through a softmax layer to get the final output
    layers.push_back(TransformerLayer(seq_len, layer_output_dims[num_layers-2], head_sizes[num_layers-1], ff_hidden_dims[num_layers-1], vocab_size));
...
void Transformer::apply_positional_encoding(std::vector<std::vector<std::vector<float>>>& input) {
...
std::vector<std::vector<std::vector<float>>> Transformer::forward(const std::vector<std::vector<std::vector<float>>>& input) {
    /*
    * Forward pass of the transformer model
    * 
    * @param input: A batch of sequences of word embeddings, (batch_size, seq_len, embedding_dim)
    * @return: A batch of sequences of word probabilities, (batch_size, seq_len, vocab_size)
    */
