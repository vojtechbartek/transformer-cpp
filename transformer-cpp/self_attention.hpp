...
public:
  ...
  std::vector<std::vector<std::vector<float>>>
  getWeights() const; // returns {Wq, Wk, Wv}
...
private:
  ...
  std::vector<std::vector<float>> Wq, Wk,
      Wv; // Weights for query, key and value
  std::vector<std::vector<float>> grad_Wq, grad_Wk,
      grad_Wv; // Gradients for query, key and value
};
